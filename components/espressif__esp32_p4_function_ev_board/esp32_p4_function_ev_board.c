...
esp_err_t bsp_spiffs_mount(void)
{
    esp_vfs_spiffs_conf_t conf = {
        ...
        .format_if_mount_failed = true      // <-- ключевое изменение
    };

    esp_err_t ret_val = esp_vfs_spiffs_register(&conf);

    if (ret_val != ESP_OK) {
        ESP_LOGW(TAG, "SPIFFS mount failed, trying explicit format...");
        ret_val = esp_spiffs_format(CONFIG_BSP_SPIFFS_PARTITION_LABEL);
        if (ret_val == ESP_OK) {
            ret_val = esp_vfs_spiffs_register(&conf);
        }
    }
